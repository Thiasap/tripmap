...
- 手机/PC 自适应：窄屏（<768px）切紧凑布局——地图固定 42vh，卡片改为地图下方两列可滚动列表，禁用卡片拖拽与图钉连线
...
- 2026-09-18 完成响应式改造：此前窗口 resize 后会 `location.reload()` 整页刷新，导致编辑中的内容被重置；现改为原地重排投影/卡片 + 窄屏紧凑布局（方案文档执行完即删，不留在 `.agent/`）
