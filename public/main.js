...
// 留白固定 120px，与历史版本逐像素一致——移动端不做单独布局（见 index.html 的 viewport），
// 保证「卡片和地图的位置永远不变」
function fitProjection(width, height) {
  state.fittedWidth = width;
  state.projection = d3.geoIdentity().reflectY(true)
    .fitExtent([[120, 12], [width - 120, height - 12]], state.geojson);
...
async function deleteSelected() {
  ...
  if (!confirm(`删除“${state.selected.name}”？卡片立即删除，媒体文件将在下次清理时自动回收。`)) return;
...
window.addEventListener('resize', () => {
  ...
  // 只对宽度变化重排：手机软键盘、地址栏收起只改高度，忽略以免输入时布局跳动
